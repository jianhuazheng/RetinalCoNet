...
from nets.RetinalCoNet_model import RetinalCoNet

from nets.training_utils import get_lr_scheduler, set_optimizer_lr, weights_init
...
if __name__ == "__main__":
    ...
    #       Enter the following in the terminal    CUDA_VISIBLE_DEVICES=0,1 python train.py
    #   DDP mode:
    ...
    model = RetinalCoNet(num_classes=num_classes, pretrained=pretrained, backbone=backbone).train()
    if not pretrained:
        weights_init(model)
    if model_path != '':
        # ------------------------------------------------------#
